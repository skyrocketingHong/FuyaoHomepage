...
下列 `/srv/fuyao` 是本项目部署模板约定的示例根目录，不是仓库检出后自动生成的目录，也不是 SvelteKit 的固定要求。服务器管理员需要手工创建它；使用其他根目录时，应同时修改 `FUYAO_*` 变量以及 systemd、Caddy 配置中的对应路径。
...
旧 `.env` 配置、部署变量、密钥边界、博客与足迹从 3.0.0 迁移到 27.0 的完整字段映射、排除项、校验和回滚要求见 [3.0.0 至 27.0 配置与内容迁移](./MIGRATION_3_TO_27_ZH.md)。迁移脚本必须通过 `FUYAO_LEGACY_ROOT` 读取独立的 3.0.0 文件树，不依赖 27.0 仓库中已经删除的 `static/posts` 与 `static/data`。
