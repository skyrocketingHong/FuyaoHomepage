<script lang="ts">
	...
	let categoryList = $derived.by(() => {
		...
		// 默认：显示 "全部" + 一级分类
		return [{ slug: 'All', title: $t('blog.all') }, ...topLevelCategories];
	});
	let postMap = $state<Record<string, Post>>({});
	...
	onMount(async () => {
		...
		actionsId = headerState.setRight(
			...
			{
				// eslint-disable-next-line svelte/no-navigation-without-resolve
				onOpenSearch: () => void goto('/blog/search')
			},
			'blog-actions'
		);
	});
		...
		if (!selectedPost) {
			...
			if (sidebarListId && sidebarState.currentListId === sidebarListId) {
				sidebarState.updateList(sidebarListId, {
					...
					onSelect: (post: Post) => selectPost(post)
				});
			} else {
	...
	async function syncStateFromPath() {
		...
		if (foundPost) {
			...
			const catExists =
				categoryList.some((c) => c.slug === cleanPath) || cleanPath === '' || cleanPath === 'All';
			if (catExists) {
	...
	async function closePost() {
		...
		// 如果可能，主要返回分类，否则返回根路径
		const target =
			activeCategory && activeCategory !== 'All' ? `/blog/${activeCategory}/` : '/blog/';
		// eslint-disable-next-line svelte/no-navigation-without-resolve
...
<div class="relative w-full">
	...
	{:else if !selectedPost && !isSearchOpen}
		...
		<Home {posts} {activeCategory} activeTag={activeTagFromPath} categories={fetchedCategories} />
	{/if}
	...
	{#if selectedPost && !isSearchOpen}
		<div class="relative h-full w-full overflow-hidden">
			<Viewer post={selectedPost} onClose={closePost} categories={fetchedCategories} />
		</div>
	{/if}
</div>
