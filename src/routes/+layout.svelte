...
{#if showContent}
	<div
		class="pointer-events-none min-h-screen font-sans transition-all duration-300 {backgroundState.component ||
	...
	>
		...
		<div class="flex h-screen w-full overflow-hidden">
			<!-- PC/平板：左侧侧边栏 (仅在桌面端显示) -->
			<div class="z-controls relative hidden pt-2 pb-2 pl-2 md:block lg:pt-4 lg:pb-4 lg:pl-4">
				<Sidebar {backgroundMode} />
			</div>
	...
	<!-- 全局加载遮罩 (Sidebar/Header层级之下，内容层级之上) -->
	<!-- 全局加载遮罩：使用 transition:fade 实现淡入淡出 -->
	{#if navigating.to}
		<div
			class="z-mask pointer-events-none fixed inset-0 flex size-full items-center justify-center bg-background/50 backdrop-blur-sm"
